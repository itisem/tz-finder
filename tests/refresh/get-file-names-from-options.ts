...
describe("getFileNamesFromOptions", () => {
	test("no options", () => {
		expect(getFileNamesFromOptions()).toMatchObject({
			url: "https://github.com/evansiroky/timezone-boundary-builder/releases/download/2023d/timezones-with-oceans.geojson.zip",
	...
	test("no oceans", () => {
		expect(getFileNamesFromOptions({release: "2022a", includeOceans: false})).toMatchObject({
			url: "https://github.com/evansiroky/timezone-boundary-builder/releases/download/2022a/timezones.geojson.zip",
			unzipped: "combined.json"
		});
	});
	test("no oceans 1970", () => {
		expect(getFileNamesFromOptions({release: "2022a", since: "1970", includeOceans: false})).toMatchObject({
	...
	test("no oceans now", () => {
		expect(getFileNamesFromOptions({release: "2022a", since: "now", includeOceans: false})).toMatchObject({
